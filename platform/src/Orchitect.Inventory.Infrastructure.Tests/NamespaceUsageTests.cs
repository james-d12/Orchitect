...
namespace Orchitect.Inventory.Infrastructure.Tests;

public sealed class NamespaceUsageTests
{
    private static readonly string[] AllowedNamespaces =
    {
        "Orchitect.Inventory.Infrastructure.Shared",
        "Orchitect.Inventory.Infrastructure.Discovery"
    };
    ...
    private static string GetRootPath()
    {
        ...
        return Path.Combine(solutionDir, "Orchitect.Inventory.Infrastructure");
    }
    ...
    public void UsingStatements_ShouldOnlyAllowThirdPartySelfAndSharedUsings(string moduleName)
    {
        ...
        foreach (var file in files)
        {
            ...
            {
                if (!usedNamespace.StartsWith("Orchitect.Inventory.Infrastructure")) continue;

                var isAllowed = usedNamespace.StartsWith($"Orchitect.Inventory.Infrastructure.{moduleName}.") ||
